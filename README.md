...
RDBMS is the database its self that uses a relational model of data while SQL is the language used to interface with it

1. Why do tables need a `primary key`?

Primary key or id is the best way to idenitfy a peice of data because it is unique to all the data.

1. What is the name given to a table column that references the primary key on another table.

Foregin key(probably misspelled that)

1. What do we need in order to have a _many to many_ relationship between two tables.

Each record in a join table includes a match field that contains the value of the primary keys of the two tables it joins.
